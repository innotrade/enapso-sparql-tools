...
for(let i=0;i<relationData.result.length;i++)
{
    ...
    {
    for(let j=0;j<relationData.result[i].child.length;j++)
    {
       if(brackets)
       {
           brackets=`${brackets} }`;
    ...
    relationData.result[i].child.forEach(element => 
        ...
        for (let i = 0; i < joins.length; i++) {
            ...
            if (joins[i].child2MasterRelation) {
                ...
                for (let prop of joins[i].cls.getProperties()) {
                    propArray = checkRelation.includes(prop.getName());
                    if (propArray == false) {
                        if(!fields.includes(' ?' + this.toLowerCase(cls.getName()) + this.toUpperCase(prop.getName())))
                        {
                            fields += ' ?' + this.toLowerCase(cls.getName()) + this.toUpperCase(prop.getName());
                        }                    } 
                }
                ...
                // create the query
let childProp = `?${this.toLowerCase(cls.getName())} ${propIRI}  ?ind.
{
	bind(?${this.toLowerCase(cls.getName())} as ?${this.toLowerCase(cls.getName())}Iri)`;
                // now add  properties
                for (let prop of cls.getProperties()) {
                    ...
                    if (propArray == false) {
                        ...
                        {
                        childProp = `${childProp}
	optional{?${this.toLowerCase(cls.getName())}  ${propStr} ?${this.toLowerCase(cls.getName())}${this.toUpperCase(prop.getName())}.}`;
                    } }
...
getAllRelation(obj,cls){
  ...
  let fieldArray=[];
    ...
    for(let i=0;i<obj.length;i++)
    {
        ...
        if(obj[i].joins)
        {
         let res=this.getInnerRelation([obj[i]],fieldArray,subChildArray);
         result.push({parent:res1.firstLevelChildDetail[i],child:res.second})
         if(res.field)
         {
   ...
   return {fields,result};
        ...
        for (let i = 0; i < child.length; i++) {
            ...
            if (child[i].child2MasterRelation) {
                ...
                for (let prop of child[i].cls.getProperties()) {
                    propArray = checkRelation.includes(prop.getName());
                    if (propArray == false) {
                        if(!fields.includes(' ?' + this.toLowerCase(cls) + this.toUpperCase(prop.getName())))
                        {
                        fields += ' ?' + this.toLowerCase(cls) + this.toUpperCase(prop.getName());
                    } 
                    }
                }
    ...
    {`;
                for (let prop of child[i].cls.getProperties()) {
                    ...
                    if (!propArray) {
                        ...
                        {
                        childDe = `${childDe}
    optional{?${this.toLowerCase(child[i].cls.getName())}  ${propStr} ?${this.toLowerCase(child[i].cls.getName())}${this.toUpperCase(prop.getName())}.}`;
                    } 
                }
                }
                childDe = `
    ...
    union
    {            
    ?${this.toLowerCase(parent.getName())}  ${propIRI}+ ?${this.toLowerCase(cls)}.
    {
    bind(?${this.toLowerCase(cls)} as ?${this.toLowerCase(cls)}Iri)`;
    ...
    if(!subChild.includes(childDe))
    {
        subChild.push(childDe);
            ...
            if(child[i].joins)
            {
                ...
                this.SubChildJoin(child[i].joins,child[i].cls,subFields,subChild)
            }
        }
        ...
        for (let prop of cls.getProperties()) {
            ...
            if (undefined != val) {
                ...
                let predicateStr = prefixPredicates
                    ...
                    {
                        fields = `${fields}\n${iriStr} ${predicateStr} ?${prop.getName()} .`;
                    }
...
${fields}
            ...
            for (let i = 0; i < joins.length; i++) {
                ...
                if (joins[i].child2MasterRelation) {
                    ...
                    let propIRI = prefixPredicates
                        ...
                        : this.prefixManager.getSparqlIRI(relation);
                    // create the query
                    let childProp = `?${this.toLowerCase(cls)} ${propIRI}  ?master.
	...
	?s ?p ?o`;
                    ...
                    let propIRI = prefixPredicates
                        ? this.prefixManager.fullToPrefixedIRI(relation)
                        : this.prefixManager.getSparqlIRI(relation);
                    var childProp = `?master  ${propIRI} ?${this.toLowerCase(cls)}.
	bind(?${this.toLowerCase(cls)} as ?s)
    ?s ?p ?o`;
                    childern.push(childProp);
                }
        ...
        for (let i = 0; i < childDetails.length; i++) {
            ...
            if (childDetails[i].child2MasterRelation) {
                ...
                let propIRI = prefixPredicates
                    ? this.prefixManager.fullToPrefixedIRI(relation)
                    : this.prefixManager.getSparqlIRI(relation);
                var child = `?${this.toLowerCase(cls)} ${propIRI}  ?${this.toLowerCase(parentClass)}.
	bind(?${this.toLowerCase(cls)} as ?s)
        ?s ?p ?o`;
                subChild.push(child);
                ...
                let propIRI = prefixPredicates
                    ? this.prefixManager.fullToPrefixedIRI(relation)
                    : this.prefixManager.getSparqlIRI(relation);
                var child = `?${this.toLowerCase(parentClass)}  ${propIRI} ?${this.toLowerCase(cls)}.
	bind(?${this.toLowerCase(cls)} as ?s)
        ?s ?p ?o`;
                subChild.push(child);
            }
