...
where {
    ...
    getIndividualsByClass(args) {
        ...
        for (let prop of cls.getProperties()) {
            ...
            if (propArray == false) {
                if (!fields.includes(this.removeIRI(prop.getName()))) {
                    fields += ' ?' + this.removeIRI(prop.getName());
                }
            }
        }
...
filter(regEx(?${fieldName}, "${filterArgs.$regex}", "${filterArgs.$options}"))`;
    ...
    bind(str(?ind) as ?iri) .`;
        for (let prop of cls.getProperties()) {
            let propStr = this.prefixManager.fullToPrefixedIRI(prop.getName());
            propArray = checkRelation.includes(prop.getName());
            if (propArray == false) {
                if (!parentClassPart.includes(propStr)) {
                    parentClassPart = `${parentClassPart}
    optional {?ind ${propStr} ?${this.removeIRI(prop.getName())} }.`;
                }
            }
        }
        parentClassPart = `${parentClassPart}
                ...
                )} ${propIRI}  ?ind.
{
	bind(?${this.toLowerCase(cls.getName())} as ?${this.toLowerCase(
                ...
                for (let prop of cls.getProperties()) {
                    let propStr = prefixPredicates
                        ? this.prefixManager.fullToPrefixedIRI(prop.getName())
                        : this.prefixManager.getSparqlIRI(prop.getName());
                    propArray = checkRelation.includes(prop.getName());
                    if (propArray == false) {
    ...
    bind( "${parent.getName()}.${cls}" as ?contentKey)
            ...
            } else if (!child.parent2ChildRelation) {
...
${conditions}
	...
	?s ?p ?o`;
                    ...
                    let relation = joins[i].parent2ChildRelation;
                    // todo: add relation prefix properties
                    let propIRI = this.prefixManager.fullToPrefixedIRI(
                        relation
                    );
