...
${conditions}
            ...
            if (propArray == false) {
                if (!fields.includes(`${prop.getName()})`)) {
                    ...
                    GROUP_CONCAT(DISTINCT
                        IF(ISIRI(?${prop.getName()}),
                            CONCAT('"', STR(?${prop.getName()}), '"'),
                            IF(ISNUMERIC(?${prop.getName()}),
                                STR(?${prop.getName()}),
                                IF(LANG(?${prop.getName()}), 
                                CONCAT('{"literal":"', STR(?${prop.getName()}), '","lang":"', LANG(?${prop.getName()}), '"}'),
                                    CONCAT('"', STR(?${prop.getName()}), '"')
                                )
                            )
                        ); SEPARATOR=", "),
    ...
    bind ("${cls.getName()}" as ?contentKey ).
        ...
        let where = `?ind sesame:directType ${clsStr}.`;
        var sparql = `select distinct ${fields}
where
  {
      ${graph.graphStatement}
