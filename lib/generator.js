 ...
 */
    ...
    deleteResourceTemplate(args) {
        let prefixPredicates = true;
        let childern = [];
        // get the join nested objects array from argument
        let joins = args.joins;
        let graph = this.findTemplateGraph(args.graph, 'cud');
...
`;
        ...
        let sparql = `delete {${graph.graphStatement}
        ?s ?p ?o ${graph.graphClosingBracket}
    }
where
{
${graph.graphStatement}
 ...
 */
    ...
    deleteRelation(master, property, child, graph) {
        ...
        property = this.prefixManager.getSparqlIRI(property);
        child = this.prefixManager.getSparqlIRI(child);
        graph = this.findGraph(graph,'cud');
...
optional{ ?class rdfs:label ?label.
        ...
        optional{?class ${this.subClass} ?superClass.
...
optional{ ?class rdfs:comment ?comment.}
          ?class ${this.subClass} ?superClass.
        filter(!isBlank(?superClass) && ?superClass!=owl:Thing)
...
optional{ ?class rdfs:label ?label.}
optional{ ?class rdfs:comment ?comment.}
optional{ ?class ${this.subClass} ?parentClass.}
        ?class rdfs:subClassOf ?superClass.
     filter(!isBlank(?parentClass))
       filter(?superClass = ${cls} && !isBlank(?class))`;
        ...
        for (let prop of cls.getProperties()) {
            ...
            if (undefined != val) {
                ...
                if (!fields.includes(propIRI)) {
                    fields = ` 
                    ${fields}
                    \${((typeof ${val} === 'string' ? ['${val}'] : typeof ${val} === 'number' ? [${val}] : typeof ${val} === 'object' && !Array.isArray(${val}) ? [${val}] : ${val}))
                      .filter(item => item !== '')
                      .map(item => {
                        if (typeof item === 'string') {
                            if(item.startsWith('http://'))
                            {
                                return \`?iri ${propIRI} <\${item}>.\`;
                            }
                            else
                            {
                                return \`?iri ${propIRI} '\${item}'.\`;
                            ...
                            return \`?iri ${propIRI} '\${item.literal}'@\${item.lang}.\`;
                    ...
                    \${((typeof ${val} === 'string' ? ['${val}'] : typeof ${val} === 'number' ? [${val}] : typeof ${val} === 'object' && !Array.isArray(${val}) ? [${val}] : ${val}))
                      ...
                      .map(item => {
                        if (typeof item === 'string') {
                            ...
                            return \`?iri ${predicateStr} '\${item.literal}'@\${item.lang}.\`;
...
${graph.graphClosingBracket}
    ...
    getIndividualsByClassTemplate(args) {
        args = args || {};
        if (!args.cls) {
            throw 'No classs passed';
        }
        ...
        args.prefixPopulations =
            args.prefixPopulations === undefined
                ...
                : args.prefixPopulations;
        let offset = args.offset === undefined ? 0 : args.offset;
        let graph = this.findTemplateGraph("graph");
        let limit = args.limit === undefined ? 0 : args.limit;
        let joins = args.joins;
        let cls = args.cls;
        let checkRelation = [];
        ...
        for (let prop of cls.getProperties()) {
            ...
            if (propArray == false) {
                if (!fields.includes(`${prop.getName()})`)) {
                    fields += ` ?${prop.getName()}`;
                    let propStr = this.prefixManager.fullToPrefixedIRI(prop.getIRI());
                    let key=prop.restriction.key;
                    let value=prop.restriction.value;
                    if ((key === "exactly" && value === 1) || (key === "max" && value === 1)) {
                        whereStatement +=`optional {?iri ${propStr} ?${prop.getName()}}`
                        filterStatement +=`\${(() => { 
                            if (Array.isArray(\${filter})) { 
                              return \${filter}.reduce((acc, obj) => { 
                                const value = obj['${prop.getName()}'];
                                if (obj.hasOwnProperty('${prop.getName()}')) {
                                  acc.push(\`FILTER(str(?${prop.getName()})=str('\${value}'))\`);
                                }
                                return acc; 
                              }, []).join('\n'); 
                            } 
                    ...
                    GROUP_CONCAT(DISTINCT
                        IF(ISIRI(?${prop.getName()}1),
                            CONCAT('"', STR(?${prop.getName()}1), '"'),
                            IF(ISNUMERIC(?${prop.getName()}1),
                                STR(?${prop.getName()}1),
                                IF(LANG(?${prop.getName()}1), 
                                CONCAT('{"literal":"', STR(?${prop.getName()}1), '","lang":"', LANG(?${prop.getName()}1), '"}'),
                                    CONCAT('"', STR(?${prop.getName()}1), '"')
                                )
                            )
                        ); SEPARATOR=", "),
                    "]") AS ?${prop.getName()}) `;
                    let wherePart=`?iri ${this.type} ${clsStr}.
                    optional {?iri ${propStr} ?${prop.getName()}1 }.`;  
                    subQueries += `{ select ${subFields} where { ${graph.graphStatement} ${wherePart} ${graph.graphClosingBracket} } group by ?iri }`
                    filterStatement +=`\${(() => { 
                        if (Array.isArray(\${filter})) { 
                          return \${filter}.reduce((acc, obj) => { 
                            const value = obj['${prop.getName()}'];
                            if (obj.hasOwnProperty('${prop.getName()}')) {
                              acc.push(\`?iri ${propStr} ?${prop.getName()}1.\nFILTER(str(?${prop.getName()}1)=str('\${value}'))\`);
                            }
                            return acc; 
                          }, []).join('\n'); 
                        } 
...
where
  {
    ${graph.graphStatement}
    ?iri ${this.type} ${clsStr}.
  \${(() => { 
        if (Array.isArray(\${filter})) { 
          return \${filter}.reduce((acc, obj) => { 
            for (let [key, value] of Object.entries(obj)) { 
              if (typeof key === 'string' && key.startsWith('<') && key.endsWith('>')) { 
                acc.push(\`<\${value}> \${key} ?iri.\`); 
              } 
            } 
            return acc; 
          }, []).join('\n'); 
...
`;
    ...
    escapeSpecialCharacters(text) {
        ...
        for (const [char, escapedChar] of Object.entries(specialCharacters)) {
