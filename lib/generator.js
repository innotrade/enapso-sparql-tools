...
where {
    ...
    */
	...
	getIndividualsByClass(args) {
		...
		args.prefixPopulations = (args.prefixPopulations === undefined ? true : args.prefixPopulations);
...
where {
	${where}
		...
		else
		{
			...
			let cls2=populations[i].cls2;
			// get the relation which store in relation
			let relation=populations[i].relation;
			// first generate list of fields class 1 to be queried
			var fields = '?'+cls.getName()+'iri';
			for (let prop of cls.getProperties()) {
				//check if the property is the relation which according to we want to join then skip that field in this class
				for(let i=0;i<populations.length;i++)
				{
					if(prop.getName()!=populations[i].relation)
					{
				...
				//  generate list of fields of class 2 to be queried
			 var fields2= '?'+cls2.getName()+'iri';
			for (let prop2 of cls2.getProperties()) {
				fields2 += ' ?' +cls2.getName()+ prop2.getName();
			}
			// check if iri filter needs to be applied for class 1
			let iris = args.iris;
			let filters = args.filter;
			let iriFilterStr = '';
			if (iris) {
		...
		filter(?ind in (${filterArgs.join(', ')})) .`;
			...
			// now generate the where clause for the select statement
			// first add the individual
			// then add the iri filter, if such is requested in class 1
			let clsStr = (args.prefixPredicates
				? this.prefixManager.fullToPrefixedIRI(cls.getIRI())
				: this.prefixManager.getSparqlIRI(cls.getIRI())
			);
		...
		`?ind rdf:type + ${clsStr} .\t${iriFilterStr}`;
			// now add all properties of class for binding 
			for (let prop of cls.getProperties()) {
				let propStr = (args.prefixPredicates
					? this.prefixManager.fullToPrefixedIRI(prop.getIRI())
					: this.prefixManager.getSparqlIRI(prop.getIRI())
		...
		${fieldFilterStr}`;
			...
			// and compose the query
			// get the fields of second class for query
			var part2 =
`?ind ${this.prefixManager.fullToPrefixedIRI(relation)} ?${cls2.getName()}iri .`;
			// now add all properties
			for (let prop2 of cls2.getProperties()) {
				let propStr2 = (args.prefixPredicates
					? this.prefixManager.fullToPrefixedIRI(prop2.getIRI())
					: this.prefixManager.getSparqlIRI(prop2.getIRI())
				);
				part2 =
					`${part2}
	        ?${cls2.getName()}iri  ${propStr2} ?${cls2.getName()}${prop2.getName()} .`;
			}
			part2 =
			`${part2}
		 bind ("${cls2.getName()}" as ?contentkey ).`;
		}
			let sparql =
				`select ${fields} ${fields2}
	where {
		${where}
		{
		{
