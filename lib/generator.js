...
where {
	${where}
		...
		else
		{
			...
			allChildClassesFields.push(fields2);
...
var childClassesPart = `?ind ${this.prefixManager.fullToPrefixedIRI(relation)} ?${cls2.getName()}iri.`;
			// now add all properties
			for (let prop2 of cls2.getProperties()) {
		...
		let sparql =
		//get all fields of child classes from an array using join
				`select ${fields} ${allChildClassesFields.join(' ')}
	where {
		${where}
		{
		    {
