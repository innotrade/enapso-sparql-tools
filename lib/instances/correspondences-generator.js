 ...
 * @param {string} baseIRI - Base IRI for the new instace, if not given the IRI of the classObject is take it.
 */
function read(classObject, records, baseIRI, idProperty, similarity = 1, separatorIRI = '#') {
	const valid = Joi.validate({ classObject, records, baseIRI, idProperty, similarity, separatorIRI }, inputSchema)
	...
	for (key of recordKeys) {
		let correspondence = findModelAlignament(key, inputs.record[key],
			inputs.sparqlJsonSchema, inputs.similarityRestriction, inputs.characterIRISeparator);
